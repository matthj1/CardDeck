    ...
    for(const [index, response] of responses.entries()){
        if(response.status === "fulfilled"){
            ...
            if(sources[index].url in cache){
                let cachedData = cache[sources[index].url].data as IInputData[]
                let chachedWithStatus = cachedData.map(sd => {return {...sd, status: StatusChoices.STALE}})
                let obj = {url: sources[index].url, status: undefined, data: chachedWithStatus}
                output = [...output, obj]
            }
        }
    }
